    ...
    try:
        ...
        while True:
            ...
            if not encrypted_message:
                break
            broadcast_encrypted_message(client_socket, clients_info, encrypted_message, name)

    except Exception as e:
        print("Error:", e)

    finally:
        # If client sends a QUIT message, remove its entry from the dictionary and broadcast updated clients info
        if name == "QUIT":
            del clients_info[client_name]
            broadcast_clients_info(client_socket, clients_info)
